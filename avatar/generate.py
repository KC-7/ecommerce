...
def get_punk_and_attributes(punk_type):
    ...
    punk_path_mapping = {
        'male': os.path.join(settings.MEDIA_ROOT, f"punks/{random.choice(punks[0:4])}"),
        'female': os.path.join(settings.MEDIA_ROOT, f"punks/{random.choice(punks[4:8])}"),
        'alien': os.path.join(settings.MEDIA_ROOT, f"punks/{punks[8]}"),
        'ape': os.path.join(settings.MEDIA_ROOT, f"punks/{punks[9]}"),
        'zombie': os.path.join(settings.MEDIA_ROOT, f"punks/{punks[10]}")
    }
    return Image.open(punk_path_mapping[punk_type]), attrDict_mapping[punk_type]


def generate_and_save_punk_for_user(user, request):
    # user = User.objects.get(pk=user.pk)
    punkTypes = ['male', 'female', 'alien', 'ape', 'zombie']
    # punks = os.listdir(os.path.join(settings.MEDIA_ROOT, 'punks'))
    backgrounds = os.listdir(os.path.join(settings.MEDIA_ROOT, 'backgrounds'))
    smokes = os.listdir(os.path.join(settings.MEDIA_ROOT, 'attributes/uni/smoke'))
    ...
    # punk_image = random.choice(punks[punkTypes.index(punkType) * 4 : (punkTypes.index(punkType) + 1) * 4])
    # punk_path = os.path.join(settings.MEDIA_ROOT, f"punks/{punk_image}")

    # # Check if the file exists.
    # if not os.path.exists(punk_path):
    #     raise FileNotFoundError(f"File not found: {punk_path}")
    ...
    if punkType == 'male':
        ...
        punkStack = Image.open(os.path.join(settings.MEDIA_ROOT, f"punks/{random.choice(punks[0:4])}"))
        metadata['Punk Type'] = 'Male'
    elif punkType == 'female':
        attrDict = prob.femaleAttr
        punkStack = Image.open(os.path.join(settings.MEDIA_ROOT, f"punks/{random.choice(punks[4:8])}"))
        metadata['Punk Type'] = 'Female'
    elif punkType == 'alien':
        attrDict = prob.alienAttr
        punkStack = Image.open(os.path.join(settings.MEDIA_ROOT, f"punks/{punks[8]}"))
        metadata['Punk Type'] = 'Alien'
    elif punkType == 'ape':
        attrDict = prob.apeAttr
        punkStack = Image.open(os.path.join(settings.MEDIA_ROOT, f"punks/{punks[9]}"))
        metadata['Punk Type'] = 'Ape'
    elif punkType == 'zombie':
        attrDict = prob.zombieAttr
        punkStack = Image.open(os.path.join(settings.MEDIA_ROOT, f"punks/{punks[10]}"))
    ...
    if hasHeadAttr:
        ...
        headChoice = Image.open(os.path.join(settings.MEDIA_ROOT, np.random.choice(headAttrs, p=list(attrDict['head'].values()))))
    ...
    if punkType == 'male' or punkType == 'zombie':
        ...
        if hasFacialHair:
            ...
            facialHairChoice = Image.open(os.path.join(settings.MEDIA_ROOT, np.random.choice(facialHairAttrs, p=list(attrDict['facialhair'].values()))))
    ...
    if hasGlasses:
        ...
        if glassesAttrs:
            glassesChoice = Image.open(os.path.join(settings.MEDIA_ROOT, np.random.choice(glassesAttrs, p=list(attrDict['eyes'].values()))))
    ...
    if hasMouthModifier:
        ...
        if mouthAttrs:
            mouthChoice = Image.open(os.path.join(settings.MEDIA_ROOT, np.random.choice(mouthAttrs, p=list(attrDict['mouth'].values()))))
    ...
    if not hasMask:
        ...
        if hasSmoke:
            smokeChoice = Image.open(os.path.join(settings.MEDIA_ROOT, f"attributes/uni/smoke/{np.random.choice(smokes, p=[0.33, 0.33, 0.34])}"))
            punkStack.paste(smokeChoice, (0, 0), smokeChoice)
            attributeCount += 1
            metadata['Smoking'] = str(smokeChoice.filename.split("/")[-1])

    if hasMask and punkType != 'ape':
        maskChoice = Image.open(os.path.join(settings.MEDIA_ROOT, f"{basedir}mask/mask01.png"))
    ...
    if hasEarring:
        earringChoice = Image.open(os.path.join(settings.MEDIA_ROOT, f"{basedir}earring/earring01.png"))
    ...
    if hasNecklace:
        ...
        if neckAttrs:
            neckChoice = Image.open(os.path.join(settings.MEDIA_ROOT, np.random.choice(neckAttrs, p=list(attrDict['neck'].values()))))
    ...
    print(f"Adding backround to your new {punkType} punk")
    punkBg = Image.open(os.path.join(settings.MEDIA_ROOT, f"backgrounds/{np.random.choice(backgrounds, p=[0.7, 0.2, 0.1])}"))
    ...
    messages.success(request, f"Successfully generated punk! Ref:({hashDigest})")
